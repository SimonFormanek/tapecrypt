...
int main(int argc, char* argv[]) { 
  ...
  sprintf(outFilNam, "%s.htmc\0", argv[1]);
  ...
  for(i = 0; i < inSizeRaw; i++) { inSize++; if((unsigned char)in[i] > 127) i++; }
  ...
  fprintf(outFil, "%i:%i\n", inSize, tapeInit); // !!! TODO VEPROVINA +1 PRO KOMPAT.S DECRYPT.SH, KDE JE TO BLBE
  
  int pos = 0, ri = 0; unsigned char idx = 0;
  for(i = 0; i < inSize; i++) { 
    ...
    //idx++; // !!!!!!! POZOR PODPURNE OPATRENI PRO (ZATIM)NEVYSVETLENOU HALUZ TODO!!!
